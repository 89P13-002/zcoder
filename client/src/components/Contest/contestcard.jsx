...
const generateTimeAndDateURL = (startTimeUnix) =>{
    ...
    const timeAndDateURL = new URL("https://www.timeanddate.com/worldclock/fixedtime.html");
    const params = {
        year: utcStartYear,
        month: utcStartMonth,
        day: utcStartDate,
...
function ContestCard({ contest }) {
    ...
    const dateTimeInUserTimezone = moment.tz(contest.startTimeUnix * 1000, userTimezone);
    const startMonth = dateTimeInUserTimezone.format("MM");
    const startDate = dateTimeInUserTimezone.format("DD");
    const startYear = dateTimeInUserTimezone.format("YYYY");
    const startTime = dateTimeInUserTimezone.format("h:mm A");

    return (
        <ContestCardContainer elevation={3}>
            <Box display="flex" justifyContent="center">
                {platformLogo && <img src={platformLogo} alt={contest.name}  />}
            </Box>
            <ContestInfo>
